...
def option_spread():
    ...
    ax.axvline(x = float(under20), color = "red", linestyle = '--', linewidth = 0.8, alpha = 1, label = "Under")
    ax.axvline(x = float(over20), color = "green", linestyle = '--', linewidth = 0.8, alpha = 1, label = "Over")
    ...
    #volume plot
    ax.plot(both_strike, volume_scaled*both_price.max(), alpha = 0.7, linewidth=0.7, label = "Volume Scaled")
    ...
    ax.text(0.99, 0.95, f"Average Over = {over_mean}", ha="right", va="top", transform=ax.transAxes, fontsize=12, color="red")
    ax.text(0.99, 0.90, f"Average Under = {under_mean}", ha="right", va="top", transform=ax.transAxes, fontsize=12, color="red")
