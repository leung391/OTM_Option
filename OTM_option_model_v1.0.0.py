...
def calculate_implied_move(calls, puts, c_p):
...
def calculate_open_interest_metrics(calls, puts):
    try:
        if calls.empty or puts.empty:
            return 0.5, 1, 1
...
def option_spread():
    ...
    volume_ratio = get_volume_profile(ticker)
    ...
    implied_move = calculate_implied_move(calls, puts, c_p)
    oi_ratio, call_vtoi, put_vtoi = calculate_open_interest_metrics(calls, puts)
    ...
    call_price_mean = calls["lastPrice"].mean()
    put_price_mean  = puts["lastPrice"].mean()
    price_skew = (call_price_mean - put_price_mean) / c_p
